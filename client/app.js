...
(function () {
  ...
  function updateVoiceStatusUI(data) {
    if (!voiceParticipantsEl || !data) return;
    // Update voice participants list
    const existingParticipant = Array.from(voiceParticipantsEl.children)
      .find(el => el.textContent.includes(data.nickname));
    
    if (!existingParticipant && data.enabled) {
      const div = document.createElement('div');
      div.className = 'voice-participant';
      div.textContent = data.nickname + ' 🎤';
      voiceParticipantsEl.appendChild(div);
    } else if (existingParticipant && !data.enabled) {
  ...
  function updateSpeakingIndicator(data) {
    if (!voiceParticipantsEl || !data) return;
    const participant = Array.from(voiceParticipantsEl.children)
      .find(el => el.textContent.includes(data.nickname));
    
    if (participant) {
      participant.className = 'voice-participant' + (data.isSpeaking ? ' speaking' : '');
    }
  }
  ...
  btnJoin.addEventListener('click', function () {
    ...
    // Connexion au serveur - configuration Replit
    const serverUrl = window.location.hostname === 'localhost' 
      ? 'http://localhost:3000' 
      : window.location.hostname === '127.0.0.1'
      ? 'http://127.0.0.1:3000'
      : 'https://chess-vote-mvp.thermophilus.replit.app'; // URL Replit
    
    // Test de disponibilité Socket.io
    if (typeof io === 'undefined') {
      lobbyMsg.textContent = 'Chargement Socket.io...';
      return;
    }
    
    socket = io(serverUrl, {
      transports: ['websocket', 'polling'], // WebSocket prioritaire sur Replit
      upgrade: true, // Permettre upgrade vers WebSocket
      forceNew: true,
      timeout: 10000,
      reconnection: true,
  ...
  if (voiceToggleBtn) voiceToggleBtn.addEventListener('click', function () {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      showToast('Chat vocal non supporté sur ce navigateur', 'error');
      return;
    }
