...
// Stockfish engine instances (one per room for performance)
...
// Stockfish AI helper
async function getStockfishSuggestion(room, depth = 15) {
  try {
    let sf = stockfishInstances.get(room.name);
    
    if (!sf) {
      sf = Stockfish();
      stockfishInstances.set(room.name, sf);
    }
    
    // Set position
    sf.position(room.chess.fen());
        ...
        if (message.startsWith('bestmove')) {
          ...
          if (parts.length >= 2) {
            ...
            if (move) {
              resolve({
                ...move,
                evaluation: message.includes('cp') ? parseInt(message.split('cp')[1].split(' ')[0]) : 0,
                depth: depth
              });
            } else {
              reject(new Error('Stockfish suggested illegal move'));
            }
          }
        }
      };
